...
class cls_template
{
     ...
     */
	function lujing(){
		$dizhi=$_SERVER['SCRIPT_NAME'];
		$weizhi=strrpos($_SERVER['SCRIPT_NAME'],'/');
		if($weizhi!=0){
			$lujing=substr($dizhi,0,$weizhi+1);
		}else{
			$lujing='/';
		}
		return $lujing;
    ...
    function make_var($val)
    {
        ...
        else
        {
            ...
            if ($_var_name == 'smarty')
            {
				...
				/*674-676行是新加的，这让就可在模板文件中通过 {$smarty.const.常量}来引用php中定义的常量*/
    ...
    function smarty_insert_scripts($args)
    {
        ...
        foreach ($arr AS $val)
        {
            if (in_array($val, $scripts) == false)
            {
                ...
                {
                    $str .= '<script type="text/javascript" src="'.$this->lujing().'js/' . $val . '"></script>';// 前面加lujing()得到的路径值===============================================
                }
            }
        }
    ...
    function smarty_prefilter_preCompile($source)
    {
        ...
        $tmp_dir   = '/themes/' . $GLOBALS['_CFG']['template'] . '/'; // 模板所在路径     前面加个斜杆========================================================
