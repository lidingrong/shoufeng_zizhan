            ...
            else
            {
                if ($rewrite)
                {
					$sql = 'SELECT type " .
						   ...
						   " WHERE re_id = '$acid' and (type=4 or type=9) and rank=1 ";
					...
					if($type==true){
						...
						$sql = 'SELECT url " .
							   " FROM ' . $GLOBALS['ecs']->table('url') .
							   " WHERE re_id = '$acid' and type=".$type." and rank=1 ";
						$url = $GLOBALS['db']->getOne($sql);
						
						
						$sql = "select url from ". $GLOBALS['ecs']->table('url') ." where type=".$lieming." and rank=0";
