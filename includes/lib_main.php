...
function assign_ur_here($cat = 0, $str = '')
{
    ...
    {
        /* 处理有分类的 */
        if (in_array($filename, array('category', 'goods', 'article_cat', 'article' ,'tags' ,'help', 'help_cat', 'brand')))
        {
            /* 商品分类或商品 */
            ...
            /* 文章分类或文章 */
            elseif ('article_cat' == $filename || 'article' == $filename || 'tags' == $filename || 'help_cat' == $filename || 'help' == $filename)
            {
                if ($cat > 0)
                {
            ...
            if (!empty($cat_arr))
            {
                krsort($cat_arr);
                if($filename=='tags'){
						$page_title = htmlspecialchars($cat_arr[1]['cat_name']) . '_' . $page_title;
						$args       = array($key => $cat_arr[1]['cat_id']);
						$ur_here   .= ' <code>&gt;</code> <a href="' . build_uri($type, $args, $cat_arr[1]['cat_name']) . '">' .
										htmlspecialchars($cat_arr[1]['cat_name']) . '</a>';
										/* 提示：如果发现用动态地址访问 tags页面的话，会显示tags分类出来，这个是正常的。因为你那个动态地址的PHP页面是article.php 。
										所以$filename得到的值是article，因此此if条件语句不成立！ */
				}else{
