 ...
 */
function cart_goods($img_url,$type = CART_GENERAL_GOODS)
{
    ...
    foreach ($arr as $key => $value)
    {
		...
		//$goods_imgsrc = "http://img.".$_SERVER['SERVER_NAME'].$row['goods_thumb']."_190X190.jpg";
		$goods_imgsrc = $img_url.$goods_thumb."_100X100.jpg";
		$arr[$key]['goods_thumb']=get_image_path($value['goods_id'], $goods_imgsrc, true);
 ...
 * @return  array
 */
function get_cart_goods($img_url)
{
    /* 初始化 */
    $goods_list = array();
    $total = array(
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        if (($GLOBALS['_CFG']['show_goods_in_cart'] == "2" || $GLOBALS['_CFG']['show_goods_in_cart'] == "3") && $row['extension_code'] != 'package_buy')
        {
            ...
            $goods_imgsrc = $img_url.$goods_thumb."_100X100.jpg";
            $row['goods_thumb'] = get_image_path($row['goods_id'], $goods_imgsrc, true);
        }
        if ($row['extension_code'] == 'package_buy')
        {
            $row['package_goods_list'] = get_package_goods($row['goods_id']);
    ...
    else
    {
        ...
        if ($user_id > 0)
        {
            ...
            $sql = "SELECT ua.*".
                    ...
                    " WHERE u.user_id='$user_id' AND ua.address_id = u.address_id and ua.IsDefault = '1'";
