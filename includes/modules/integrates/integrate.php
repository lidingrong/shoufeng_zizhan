...
class integrate
{
    ...
    function integrate($cfg)
    {
        ...
        $this->need_sync = true;
//$control_db_host="localhost:3306";
//$control_db_user="root";
//$control_db_pass="ldr19880207";
//$this->db_name2=$control_db_name="ecshop3";
//$this->prefix2="ecs_";
        ...
        if (empty($cfg['db_host']))
        {
            ...
            $this->db = &$GLOBALS['db'];
$this->db_name2=&$GLOBALS['control_db_name'];
$this->prefix2=&$GLOBALS['prefix2'];
$this->website_id=&$GLOBALS['website_id'];
...
$this->db2 = new cls_mysql($control_db_host, $control_db_user, $control_db_pass, $control_db_name, $this->charset, NULL,  $quiet);
            }
            else
            {
                $this->db = new cls_mysql($cfg['db_host'], $cfg['db_user'], $cfg['db_pass'], $cfg['db_name'], 'latin1', NULL, $quiet) ;
$this->db2 = new cls_mysql($control_db_host, $control_db_user, $control_db_pass, $control_db_name, 'latin1', NULL, $quiet) ;
            }
        }
...
$fields2 = array("website_id", $this->field_name, $this->field_email, $this->field_pass);
//$values2 = array($GLOBALS['website_id'], $post_username, $email, $post_password);
$values2 = array($this->website_id, $post_username, $email, $post_password);
        if ($gender > -1)
        {
            $fields[] = $this->field_gender;
            $values[] = $gender;
$fields2[] = $this->field_gender;
$values2[] = $gender;
        }
        if ($bday)
        {
...
$sql2 = "INSERT INTO " . $this->table2($this->user_table).
	   " (" . implode(',', $fields2) . ")".
...
$this->db2->query($sql2);
    ...
    function set_session ($username='')
    {
        ...
        else
        {
			...
			//获取核心数据库中的用户id
			$website_id = $GLOBALS['website_id'];
            $sql2 = "SELECT user_id FROM " . $GLOBALS['ecs2']->table('users') . " WHERE user_name='$username' AND website_id='$website_id' LIMIT 1";
            $row2 = $GLOBALS['db2']->getRow($sql2);
            ...
            if ($row)
            {
				...
				//在session中存储核心数据库中的用户id
				if($row2){
					$_SESSION['control_user_id']     = $row2['user_id'];
				}
            }
        }
    }
...
function table2($str)
{
	return '`' .$this->db_name2. '`.`'.$this->prefix2.$str.'`';
}
