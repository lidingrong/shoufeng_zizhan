...
function get_userinfo($user_id){
    ...
    $sql  = "SELECT `user_id`,`email`,`address_id`,`FirstName`, `LastName`, `FirstName_2`, `LastName_2`  FROM " .$GLOBALS['ecs']->table('users') . " WHERE user_id = '$user_id'";     
           
    $info = $GLOBALS['db']->getRow($sql);
    $infos['user_name'] = addslashes($infos['user_name']);
	    
    $sql  = "SELECT `address_id`, `FirstName`, `LastName`, `FirstName_2`, `LastName_2`, `email`, `State`, `City`, `Room`, `AddressLine`, `Country`, `PostalCode`, `Phone`, `IsDefault` FROM " .$GLOBALS['ecs']->table('user_address') . " WHERE user_id = '$user_id' and  IsDefault = '1' ";             
...
function get_profile($user_id)
{
   ...
   // $sql  = "SELECT user_name, birthday, sex, question, answer, rank_points, pay_points,user_money, user_rank,". " msn, qq, office_phone, home_phone, mobile_phone, passwd_question, passwd_answer ".  "FROM " .$GLOBALS['ecs']->table('users') . " WHERE user_id = '$user_id'";
      ...
      $sql  = "SELECT `user_id`,`email`,`user_name`,`address_id`,`FirstName`,`LastName`,`FirstName_2`,`LastName_2`".  "FROM " .$GLOBALS['ecs']->table('users') . " WHERE user_id = '$user_id'";     
...
function get_consignee_list($user_id)
{
	...
	$consignee_list = $GLOBALS['db']->getRow($sql);
...
function get_user_orders($user_id, $num = 10, $start = 0)
{
    /* 取得订单列表 */
    $arr    = array();
    $sql = "SELECT order_id, order_sn, order_status, shipping_status, pay_status, add_time,goods_amount,shipping_fee, " .
...
function update_address($address)
{
    ...
    if ($address_id > 0)
    {
		...
		/*同时更新一条核心数据库中的指定记录*/
		//这个逻辑有一个需要注意的问题，子站库中的address_id和核心库中的address_id是不对应的，需要进行准确定位
		//单个地址的时候可以使用下面的数据库更新
		$address['user_id']=$_SESSION['control_user_id'];//插入核心数据库的时候把数组中的user_id换成核心库的
		$GLOBALS['db2']->autoExecute($GLOBALS['ecs2']->table('user_address'), $address, 'UPDATE', 'user_id = ' . $_SESSION['control_user_id']);
		$address['user_id']=$_SESSION['user_id'];//插入核心数据库后把数组中的user_id换成子站的
		//如果涉及到一个用户可以管理多个地址信息的时候，就使用下面的方法
		//TO DO
    }
    else
    {
		...
		/* 插入一条新记录到核心数据库 */
		$address['user_id']=$_SESSION['control_user_id'];//插入核心数据库的时候把数组中的user_id换成核心库的
        $GLOBALS['db2']->autoExecute($GLOBALS['ecs2']->table('user_address'), $address, 'INSERT');
        
		$address['user_id']=$_SESSION['user_id'];//插入核心数据库后把数组中的user_id换成子站的
    }

    if (isset($address['IsDefault']) && $address['IsDefault'] > 0 && isset($address['user_id']))
    {
        $sql = "UPDATE ".$GLOBALS['ecs']->table('users') .
