...
elseif ($_REQUEST['step'] == 'login')
{
    ...
    else
    {
        ...
        if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'signin')
        {
            ...
            if ($user->login($_REQUEST['username'], $_REQUEST['password'],0))
            {
                ...
                if ($db->getOne($sql) > 0)
                {
                    ...
                    //echo json_encode($consignee_list);                   
                   // exit();
                ...
                $result = $consignee_list;
      
                $result['error'] = '1';

                $result['content'] = 'ようこそ <a rel="nofollow" href="user.php" class="fc_red" >'.$result['FirstName'].$result['LastName'].'</a> 様: <a href="user.php?act=logout">ログアウト</a><br />';
            ...
            {
				 $sql = "SELECT COUNT(*) FROM " . $ecs->table('users') . " WHERE user_name  = '" .$_REQUEST['username'] . "' ";
				 if ($db->getOne($sql) > 0){
					 $result['error'] = '0';
				 }else{
					 $result['error'] = '2';
				...
				echo $json->encode($result);
				//show_message($_LANG['signin_failed'], '', 'flow.php');
				exit;
            }
        ...
        elseif (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'signup')
        {
            ...
            if ((intval($_REQUEST['captcha']) & CAPTCHA_REGISTER) && gd_version() > 0)
            {
            ...
            $other['FirstName'] = isset($_REQUEST['firstname']) ? trim($_REQUEST['firstname']) : '';
    		$other['LastName'] = isset($_REQUEST['lastname']) ? trim($_REQUEST['lastname']) : '';
    		$other['FirstName_2'] = isset($_REQUEST['firstname2']) ? trim($_REQUEST['firstname2']) : '';
    		$other['LastName_2'] = isset($_REQUEST['lastname2']) ? trim($_REQUEST['lastname2']) : '';   		
            $other['mobile_phone'] = isset($_REQUEST['phone']) ? $_REQUEST['phone'] : '';
    		$other['State'] = isset($_REQUEST['state']) ? trim($_REQUEST['state']) : '';
    		$other['City'] = isset($_REQUEST['city']) ? trim($_REQUEST['city']) : '';
    		$other['Room'] = isset($_REQUEST['room']) ? trim($_REQUEST['room']) : '';
    		$other['AddressLine'] = isset($_REQUEST['addressline']) ? trim($_REQUEST['addressline']) : '';
    		$other['PostalCode'] = isset($_REQUEST['postalcode']) ? $_REQUEST['postalcode'] : '';
            

            if (register(trim($_REQUEST['email']), trim($_REQUEST['password']), trim($_REQUEST['email']),$other))
            {
...
elseif($_REQUEST['step']=='update_group_cart')
    ...
    /* 取得商品列表，计算合计 */
    $cart_goods = get_cart_goods($img_url);
...
elseif ($_REQUEST['step'] == 'check')
{	
	$user_info = user_info($_SESSION['user_id']);
	$consignee = get_consignee($_SESSION['user_id']);
	if($consignee['user_id']){
		$consignee['user_id'] = $_SESSION['user_name'];
	}
    ...
    $flow_type = isset($_SESSION['flow_type']) ? intval($_SESSION['flow_type']) : CART_GENERAL_GOODS;
	 /* 对商品信息赋值 */
    $cart_goods = cart_goods($img_url,$flow_type); // 取得商品列表，计算合计
	/*
	  * 取得订单信息
     */
...
elseif ($_REQUEST['step'] == 'select_ship')
{
 	include_once('includes/cls_json.php');
    $json = new JSON;
    $result = array('error' => '', 'shipping_price' => '','totalPrice'=>'','totalPrice2'=>'');
	
	$shipping_price = $_REQUEST['shipping_price'] ?  $_REQUEST['shipping_price'] : 0;
	$totalPrice = $_REQUEST['totalPrice'] ?  $_REQUEST['totalPrice'] : 0;
	
	$result['shipping_price'] =$shipping_price.".00円";
	$result['totalPrice'] =$totalPrice+$shipping_price.".00円";
	$result['totalPrice2'] =$totalPrice+$shipping_price;
	

	$smarty->fetch('library/order_total_flows.lbi');
	echo $json->encode($result);
    exit;
}
elseif ($_REQUEST['step'] == 'select_shipping')
{
    ...
    else
    {
        ...
        $result['content']     = $smarty->fetch('library/order_total_flows.lbi');
    }
...
elseif ($_REQUEST['step'] == 'change_needinv')
{
    ...
    else
    {
        ...
        die($smarty->fetch('library/order_total_flows.lbi'));
    }
}
...
elseif ($_REQUEST['step'] == 'done')
{
	...
	$is_register ="";
	$consignee_post = "";
	$login_post  = empty($_POST['login_post']) ? 0  : intval($_POST['login_post']);
	$is_register  = empty($_POST['is_register']) ? 0  : intval($_POST['is_register']);
	$consignee_post = empty($_POST['consignee_post']) ? 0  : intval($_POST['consignee_post']);
	//直接购买
	if($login_post=='1' && !$is_register && $_SESSION['user_id']){
		$consignee = array(
            'address_id'    => empty($_POST['address_id']) ? 0  : intval($_POST['address_id']),
            'user_id'       => $_SESSION['user_id'] ,
            'FirstName'       => empty($_POST['ShippingFirstName'])    ? '' : $_POST['ShippingFirstName'],
            'LastName'      => empty($_POST['ShippingLastName'])   ? '' : $_POST['ShippingLastName'],
            'FirstName_2'          => empty($_POST['ShippingFirstName_2'])   ? '' : $_POST['ShippingFirstName_2'],
            'LastName_2'      => empty($_POST['ShippingLastName_2'])   ? '' : $_POST['ShippingLastName_2'],
            'email'         => empty($_POST['Email'])   ? '' : $_POST['Email'],
            'State'       => empty($_POST['ShippingState'])    ? '' : $_POST['ShippingState'],
            'City'       => empty($_POST['ShippingCity'])    ? '' : $_POST['ShippingCity'],
			'AddressLine'        => empty($_POST['ShippingAddressLine'])    ? '' : $_POST['ShippingAddressLine'],
            'Room'          => empty($_POST['ShippingRoom'])    ? '' : $_POST['ShippingRoom'],
            'PostalCode' => empty($_POST['ShippingPostalCode'])    ? '' : make_semiangle(trim($_POST['ShippingPostalCode'])),
            'Phone'      => empty($_POST['ShippingPhone'])     ? '' : make_semiangle(trim($_POST['ShippingPhone'])),
            'IsDefault'     => 1,
        );
	}
	//注册或是不注册购买
	if($consignee_post == '1' && $is_register && !$login_post && ! $_SESSION['user_id']){
		$user_manger = array();
		if($is_register == "1"){
			$user_manger = array(
				'mem_Password'    => empty($_POST['mem_Password']) ? 0  : $_POST['mem_Password'],
			);
		}
		$consignee = array(
            'address_id'    => empty($_POST['address_id']) ? 0  : intval($_POST['address_id']),
            'user_id'       => $_SESSION['user_id'] ,
            'FirstName'       => empty($_POST['mem_FirstName'])    ? '' : $_POST['mem_FirstName'],
            'LastName'      => empty($_POST['mem_LastName'])   ? '' : $_POST['mem_LastName'],
            'FirstName_2'          => empty($_POST['mem_FirstName_2'])   ? '' : $_POST['mem_FirstName_2'],
            'LastName_2'      => empty($_POST['mem_LastName_2'])   ? '' : $_POST['mem_LastName_2'],
            'email'         => empty($_POST['mem_Email'])   ? '' : $_POST['mem_Email'],
            'State'       => empty($_POST['mem_State'])    ? '' : $_POST['mem_State'],
            'City'       => empty($_POST['mem_City'])    ? '' : $_POST['mem_City'],
			'AddressLine'        => empty($_POST['mem_AddressLine'])    ? '' : $_POST['mem_AddressLine'],
            'Room'          => empty($_POST['mem_Room'])    ? '' : $_POST['mem_Room'],
            'PostalCode' => empty($_POST['mem_PostalCode'])    ? '' : make_semiangle(trim($_POST['mem_PostalCode'])),
            'Phone'      => empty($_POST['mem_Phone'])     ? '' : make_semiangle(trim($_POST['mem_Phone'])),
            'IsDefault'     => 1,
        );
	}
    ...
    {
        $cart_goods_stock = get_cart_goods($img_url);
    ...
    /* By long */
	if($consignee_post==1 || $login_post == 1)
	{
        ...
        if ($_SESSION['user_id'] > 0  )
        {
			...
			if (!$_CFG['shop_reg_closed'])
			{
				...
				$password = $user_manger['mem_Password'];
				$email    =  $consignee['email'];
				
				$other['FirstName'] = $consignee['FirstName'];
				$other['LastName'] = $consignee['LastName'];
				$other['FirstName_2'] = $consignee['FirstName_2'];
				$other['LastName_2'] = $consignee['LastName_2'];
				$other['mobile_phone'] = $consignee['Phone'];
				$other['State'] = $consignee['State'];
				$other['City'] = $consignee['City'];
				$other['Room'] = $consignee['Room'];
				$other['AddressLine'] = $consignee['AddressLine'];
				$other['PostalCode'] = $consignee['PostalCode'];

				$pattern = "/^([0-9A-Za-z\\-_\\.]+)@([0-9a-z]+\\.[a-z]{2,3}(\\.[a-z]{2})?)$/i";
				if ( preg_match( $pattern, $email ) == 0){
					
					 show_message("邮件地址不合法！");
				}
				...
				{
					$err->show($_LANG['sign_up'], 'flow.php?step=check');
				}
			}
    ...
    $order = array(
        'shipping_id'     => intval($_POST['ShippingSId']),
    ...
    $order['consignee'] = $consignee['FirstName'].$consignee['LastName'];
    ...
    if ($order['shipping_id'] > 0)
    {
        ...
        if($order['shipping_id']== '1'){
            //$shipping = shipping_info($order['shipping_id']);
            $order['shipping_name'] = "国際小包";
            $order['shipping_fee'] = 0;
        }
        if($order['shipping_id']== '2'){
            
            //$shipping = shipping_info($order['shipping_id']);
            $order['shipping_name'] = "EMS（国際速達郵便）";
   ...
   // $order['insure_fee']   = $total['shipping_insure'];
    ...
    //user_uc_call('add_feed', array($order['order_id'], BUY_GOODS)); //推送feed到uc
    ecs_header("Location: flow.php?step=toOrder\n"); 
    exit;
}
elseif ($_REQUEST['step'] == 'toOrder'){
    ...
    $order_id2 =  $_REQUEST['oi'];
   if($order_id > 0 || $order_id2 > 0 || $order_id2= $order_id )
   {
        //获取订单信息
        $sql = "SELECT order_id,`order_sn`,`consignee`,`country`,`State`,`City`,`Room`,`AddressLine`,`PostalCode`,`Phone`,`email`,`shipping_id`,`shipping_name`,`shipping_fee` FROM ".
               $GLOBALS['ecs']->table('order_info') .
                " WHERE order_id ='".$order_id."'";

        $res_order = $GLOBALS['db']->getAll($sql);
        $order= $res_order['0'];
        
        //获取订单中的商品
        $sql = "SELECT `order_id`,`goods_id`,`goods_name`,`goods_sn`,`goods_number`,`goods_price`,`is_real` FROM ".
        ...
        foreach($goods_list as $k=> $value){
            ...
            $total['price'] += $value['goods_price']*$value['goods_number'];
            
             $goods_list[$k]['total_price'] = $value['goods_price']*$value['goods_number'];
             $goods_imgsrc = $db->getOne("SELECT  goods_thumb FROM " .$ecs->table('goods'). " WHERE goods_id = '".$value['goods_id']."'  ");          
		     $goods_list[$k]['goods_thumb'] =$goods_imgsrc = $img_url.$goods_imgsrc."_100X100.jpg";
        }
        
        $total['total'] = $total['price']+$order['shipping_fee'];
   }else{
        ecs_header("Location: index.php\n"); 
        exit;
   }
   ...
   //ecs_header("Location: flow.php?step=over\n"); 
 
}
elseif ($_REQUEST['step'] == 'over'){
    $postscript =  $_REQUEST['postscript'];
    $order_id = $_REQUEST['order_id'];
   if($postscript){
        $sql = "UPDATE " .$GLOBALS['ecs']->table('order_info'). " SET postscript = '".$postscript."'  WHERE order_id = '".$order_id."'";          
        $db->query($sql);
   }
   //获取订单信息
        $sql = "SELECT order_id,`order_sn`,`consignee`,`country`,`State`,`City`,`Room`,`AddressLine`,`PostalCode`,`Phone`,`email`,`shipping_id`,`shipping_name`,`shipping_fee` FROM ".
    ...
    $smarty->assign('order',      $order);
    $smarty->assign('ordertime',    date('m/d/Y H:i:s'));
    unset($_SESSION['order_id']); // 清除session中保存的收货人信息
...
elseif ($_REQUEST['step'] == 'update_cart')
{
    ...
    //show_message($_LANG['update_cart_notice'], $_LANG['back_to_cart'], 'flow.php?step=checkout');
	ecs_header("Location: flow.php\n");
    exit;
}
...
//-- 修改地址信息
/*------------------------------------------------------ */
elseif($_REQUEST['step'] == 'editAddress'){
	...
	$address = array(
		...
		'FirstName'    => $_REQUEST['firstname'] ?  $_REQUEST['firstname'] :0,
		'LastName'   => $_REQUEST['lastname'] ?  $_REQUEST['lastname'] :0,
		'FirstName_2'       => $_REQUEST['firstname2'] ?  $_REQUEST['firstname2'] :0,
		'LastName_2'   =>  $_REQUEST['lastname2'] ?  $_REQUEST['lastname2'] :0,
		'State'  =>  $_REQUEST['state'] ?  $_REQUEST['state'] :0,
		'City'      => $_REQUEST['city'] ?  $_REQUEST['city'] :0,
		'Room'        => $_REQUEST['room'] ?  $_REQUEST['room'] :0,
		'AddressLine'     => $_REQUEST['addressline'] ?  $_REQUEST['addressline'] :0,
		'PostalCode'  =>$_REQUEST['postalcode'] ?  $_REQUEST['postalcode'] :0,
		'Phone' =>  $_REQUEST['phone'] ?  $_REQUEST['phone'] :0,
		'IsDefault'       => 1,
	);
	...
	$info =  get_userinfo($_SESSION['user_id']);
	$result_e['content'] = 'ようこそ <a rel="nofollow" href="user.php" class="fc_red" >'.$info['FirstName'].$info['LastName'].'</a> 様: <a href="user.php?act=logout">ログアウト</a><br />';

    $consignee_list = get_consignee_list($_SESSION['user_id']);
    $result_a = $consignee_list;
    
    $result = array_merge($result_a,$result_e);
    ...
    foreach ($arr AS $key => $val)
    {
        ...
        if (intval($GLOBALS['_CFG']['use_storage']) > 0 && $goods['extension_code'] != 'package_buy')
        {
            if ($row['goods_number'] < $val)
            {
                ...
                $row['goods_number'], $row['goods_number']),'返回上一页','flow.php?step=checkout');
                exit;
            }
            /* 是货品 */
            ...
            if (!empty($goods['product_id']))
            {
                ...
                {
                    show_message(sprintf($GLOBALS['_LANG']['stock_insufficiency'], $row['goods_name'],
                    $product_number['product_number'], $product_number['product_number']), '返回上一页', 'flow.php?step=checkout');
                    exit;
                }
            }
        }
