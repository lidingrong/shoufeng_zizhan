...
if(isset($_POST['submit']) && $_POST['submit']!='' && $_GET['do']=='ok'){
	if(isset($zzw_choose) &&  $zzw_choose!=''){
		...
		if($zzw_choose=='choose_1'){
			...
			$cole_sql1="select distinct pid from ".$GLOBALS['ecs2']->table($core_table_goods)." where pid<11 order by pid";
...
function setWater($imgSrc,$markImg,$markPos,$markType)
{
	...
	if(!strcmp($markType,"img"))
    {
        ...
        if($srcImg_w < $markImg_w )
        {
			//修改宽与高
			ImageResize($markImg,$srcImg_w,$markImg_h);
			
		}
		if($srcImg_h < $markImg_h ){
			//修改宽与高
			ImageResize($markImg,$markImg_w,$srcImg_h);
...
<div style="width:980px; margin:50px auto;">
	<div>使用必读：
		<ul>
			<li>(1)需要使用核心的init.php和config.php数据库配置！确保能正确连接到核心数据库！</li>
			<li>(2)在本程序配置下自己子站的goods表明与goods_gallery表名</li>
