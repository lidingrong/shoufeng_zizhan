    ...
    switch ($app)
    {
        case 'category':
            ...
            else
            {
                if ($rewrite)
                {
					$sql = "SELECT b.parent_id,a.url  FROM ". $GLOBALS['ecs']->table('url') ." a INNER JOIN ". $GLOBALS['ecs']->table('category') ." b ON a.re_id=b.cat_id  WHERE a.re_id = '$cid' and type=1 ";
					$row = $GLOBALS['db']->GetRow($sql);
					...
					if($row['parent_id']==0){
						...
						$sql2 = "SELECT url  FROM ". $GLOBALS['ecs']->table('url') ."  WHERE re_id = '".$row['parent_id']."' and type=1 ";
						$url = $GLOBALS['db']->getOne($sql2);
						$uri = '/'.$url.'/'.$row['url'];
					}
        ...
        case 'goods':
            ...
            {
				$sql = "SELECT u.url,c.parent_id FROM (" . $GLOBALS['ecs']->table('goods')." g INNER JOIN " . $GLOBALS['ecs']->table('category')." c ON g.cat_id=c.cat_id)
						INNER JOIN " . $GLOBALS['ecs']->table('url')." u ON g.cat_id=u.re_id  WHERE u.type=1 AND g.goods_id = '$gid' ";
				$yi_url = $GLOBALS['db']->GetRow($sql);
				
				if($yi_url['parent_id']!=0){
					$sql2 = "SELECT url FROM " . $GLOBALS['ecs']->table('url')."  WHERE type=1 AND re_id = '".$yi_url['parent_id']."' AND rank=1 ";
					$er_url = $GLOBALS['db']->getOne($sql2);
					$urll=$er_url.'/'.$yi_url['url'];//品牌 + 系列
				}else{
					$urll=$yi_url['url']; //品牌
				}
				
				$sql = "select url from " . $GLOBALS['ecs']->table('url')." where type=2 and rank=3 and re_id=0 ";
				$goods_qz = $GLOBALS['db']->getOne($sql);//前缀
				
				$uri = $rewrite ? '/'.$urll.'/'.$goods_qz.'-' . $gid : 'goods.php?id=' . $gid;
            }
        ...
        case 'article_cat':
            ...
            else
            {
                ...
                {
					$sql = 'SELECT url " .
						   " FROM ' . $GLOBALS['ecs']->table('url') .
						   " WHERE re_id = '$acid' and type=4 and rank=1 ";
					$url = $GLOBALS['db']->getOne($sql);
					
					$sql2 = 'SELECT url " .
						   " FROM ' . $GLOBALS['ecs']->table('url') .
						   " WHERE re_id = '$acid' and type=9 and rank=1 ";
					$url2 = $GLOBALS['db']->getOne($sql2);
					
					if($url==true){
						$sql = "select url from ". $GLOBALS['ecs']->table('url') ." where type=3 and rank=0";
						$article_fenlei = $GLOBALS['db']->getOne($sql);
						$uri = '/'.$article_fenlei.'/'.$url;
					}if($url2==true){
						$sql = "select url from ". $GLOBALS['ecs']->table('url') ." where type=8 and rank=0";
						$article_fenlei = $GLOBALS['db']->getOne($sql);
						$uri = '/'.$article_fenlei.'/'.$url2;
					}else{
						$sql = "select url from ". $GLOBALS['ecs']->table('url') ." where rank=0 and re_id= '$acid'";
						$article_fenlei = $GLOBALS['db']->getOne($sql);
						$uri = '/'.$article_fenlei;
					}
        ...
        case 'article':
            ...
            else
            {
				...
				if($rewrite){
					$sql = "SELECT u.url,u.type,a.cat_id  FROM (" . $GLOBALS['ecs']->table('article') . " a INNER JOIN " . $GLOBALS['ecs']->table('article_cat') . " c ON a.cat_id=c.cat_id) 
							INNER JOIN " . $GLOBALS['ecs']->table('url') . " u ON c.parent_id=u.re_id WHERE u.rank=0 AND a.article_id = '$aid' ";
					$row = $GLOBALS['db']->GetRow($sql);
					if($row['type']==3 || $row['type']==8){
						$url_type=($row['type']==3)?4:9;
						$sql2="SELECT url FROM " . $GLOBALS['ecs']->table('url') . " WHERE re_id='".$row['cat_id']."' and rank=1 and type=".$url_type;
						$url = $GLOBALS['db']->getOne($sql2);
						
						$qz_type=($row['type']==3)?5:10;
						$sql3="SELECT url FROM " . $GLOBALS['ecs']->table('url') . " WHERE type=".$qz_type." AND rank=3 ";
						$_qz = $GLOBALS['db']->getOne($sql3);
						
						$uri = '/'.$row['url'].'/'.$url.'/'.$_qz.'-'. $aid;
					}elseif($row['type']==6){
						$sql3="SELECT url FROM " . $GLOBALS['ecs']->table('url') . " WHERE type=7 AND rank=3 ";
						$tags_qz = $GLOBALS['db']->getOne($sql3);
						
						$uri = '/'.$row['url'].'/'.$tags_qz.'-'. $aid;
					}
