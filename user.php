...
/* 显示页面的action列表 */
$ui_arr = array('register', 'login', 'profile', 'order_list','act_edit_info','edit_password', 'order_detail', 'address_list', 'collection_list',
...
if ($action == 'default')
{
    ...
    if ($rank = get_rank_info())
    {
     ...
     $smarty->assign('userinfo',        get_userinfo($user_id));
	$smarty->assign('info',        get_consignee_list($user_id));
...
elseif($action == 'act_edit_info'){
	...
	if($data == "member_profile" && $address_id != ""){
		$name = array();
		$name['FirstName'] = isset($_POST['FirstName'])?trim($_POST['FirstName']):null;
		$name['LastName'] = isset($_POST['LastName'])?trim($_POST['LastName']):"";
		$name['FirstName_2'] = isset($_POST['FirstName_2'])?trim($_POST['FirstName_2']):"";
		$name['LastName_2'] = isset($_POST['LastName_2'])?trim($_POST['LastName_2']):"";
		if($name){
//			$sql = 'UPDATE ' . $ecs->table('user_address') . " SET FirstName = '".$name['FirstName']."', LastName = '".$name['LastName']."', FirstName_2 = '".$name['FirstName_2']."', LastName_2 = '".$name['LastName_2']."' WHERE address_id = '$address_id' AND user_id = '$user_id'";
//			$db->query($sql);
			
			$sql2 = 'UPDATE ' . $ecs->table('users') . " SET FirstName = '".$name['FirstName']."', LastName = '".$name['LastName']."', FirstName_2 = '".$name['FirstName_2']."', LastName_2 = '".$name['LastName_2']."' WHERE user_id = '$user_id'";
			$db->query($sql2);
			//同时更新用户信息到核心数据库
			$control_user_id = $_SESSION['control_user_id'];
			$sql2 = 'UPDATE ' . $ecs2->table('users') . " SET FirstName = '".$name['FirstName']."', LastName = '".$name['LastName']."', FirstName_2 = '".$name['FirstName_2']."', LastName_2 = '".$name['LastName_2']."' WHERE user_id = '$control_user_id'";
			$db2->query($sql2);
		}
		show_message('個人情報の変更は完了しました。','','user.php');
	}
	
	$action = 'act_edit_info';
	$smarty->assign('userinfo',        get_userinfo($user_id));
	$smarty->assign('info',        get_consignee_list($user_id));
	$smarty->assign('action',     $action);
	$smarty->assign('prompt',      get_user_prompt($user_id));
	...
	if($data == "member_password"){
		...
		if (strlen($Password) < 6)
		{
		 ...
		 if($Password != $ConfirmPassword){
			 show_message('入力された二つのパスワードが一致していません。');
		 }

		if ($user->edit_user(array('username'=> (empty($code) ? $_SESSION['user_name'] : $info['email']), 'old_password'=>$old_password, 'password'=>$Password), empty($code) ? 0 : 1))
        {
...
elseif($action == 'upd_shipping_address'){
	include_once(ROOT_PATH . 'includes/lib_transaction.php');
	$data = isset($_POST['data'])?trim($_POST['data']):"";
	$address_id = isset($_POST['address_id'])?trim($_POST['address_id']):"";
	
	if($data == "member_address_book" && $address_id != ""){
		
		$FirstName = isset($_POST['FirstName'])?trim($_POST['FirstName']):"";
		$LastName = isset($_POST['LastName'])?trim($_POST['LastName']):"";
		$FirstName_2 = isset($_POST['FirstName_2'])?trim($_POST['FirstName_2']):"";
		$LastName_2 = isset($_POST['LastName_2'])?trim($_POST['LastName_2']):"";
...
//			
//			$sql2 = 'UPDATE ' . $ecs->table('users') . " SET FirstName = '".$FirstName."', LastName = '".$LastName."', FirstName_2 = '".$LastName."', LastName_2 = '".$LastName_2."',mobile_phone  = '".$Phone."'  WHERE address_id = '$address_id' AND user_id = '$user_id'";
//			$db->query($sql2);
...
elseif ($action == 'act_register')
{
    ...
    else
    {
		...
		$other['FirstName'] = isset($_POST['FirstName']) ? trim($_POST['FirstName']) : '';
		$other['LastName'] = isset($_POST['LastName']) ? trim($_POST['LastName']) : '';
		$other['FirstName_2'] = isset($_POST['FirstName_2']) ? trim($_POST['FirstName_2']) : '';
		$other['LastName_2'] = isset($_POST['LastName_2']) ? trim($_POST['LastName_2']) : '';
		
        $other['mobile_phone'] = isset($_POST['mobile_phone']) ? $_POST['mobile_phone'] : '';
		$other['State'] = isset($_POST['State']) ? trim($_POST['State']) : '';
		$other['City'] = isset($_POST['City']) ? trim($_POST['City']) : '';
		$other['Room'] = isset($_POST['Room']) ? trim($_POST['Room']) : '';
		$other['AddressLine'] = isset($_POST['AddressLine']) ? trim($_POST['AddressLine']) : '';
		$other['PostalCode'] = isset($_POST['PostalCode']) ? $_POST['PostalCode'] : '';
...
elseif($action == 'check_email')
{
   ...
   $email = trim($_GET['email']);
	// $email=  json_str_iconv(trim($_GET['email']));
...
elseif ($action == 'send_pwd_email')
{
	...
	$user_name = !empty($_POST['email'])     ? trim($_POST['email'])     : '';
    //用户名和邮件地址是否匹配
...
elseif ($action == 'order_detail')
{
    ...
    $order['shipping_status'] = $_LANG['ss'][$order['shipping_status']];
	 ...
	 $order['total_price'] = $order['goods_amount']+$order['shipping_fee'];

//	$smarty->assign('info',        get_userinfo($user_id));	 
...
elseif ($action == 'collection_list')
{
    ...
    $smarty->assign('goods_list', get_collection_goods($img_url,$user_id, $pager['size'], $pager['start']));
